...
if (isset($_POST['Submit'])) {
	...
	if ($_POST['id']) {
		...
		if( !is_wp_error($user_id) ) {
			update_user_meta( $user_id, 'gender', 		$_POST['atributes']['gender'] );
			update_user_meta( $user_id, 'national_id', 	$_POST['atributes']['national_id'] );
			update_user_meta( $user_id, 'gender', 		$_POST['atributes']['gender'] );
			update_user_meta( $user_id, 'birthDate', 	$_POST['atributes']['birthDate'] );
			update_user_meta( $user_id, 'active', 1, true );
		...
		// $user->add_cap("Mnbaa_Tamarji_patient");
		
		//On success
		if( !is_wp_error($user_id) ) {
			add_user_meta( $user_id, 'gender', 		$_POST['atributes']['gender'], true );
			add_user_meta( $user_id, 'national_id', $_POST['atributes']['national_id'], true );
			add_user_meta( $user_id, 'gender', 		$_POST['atributes']['gender'], true );
			add_user_meta( $user_id, 'birthDate', 	$_POST['atributes']['birthDate'], true );
...
// include view
include (plugin_dir_path(__DIR__) . 'views/patient.php');
?>